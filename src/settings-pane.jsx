...
var SettingsPane = React.createClass({
  ...
  render: function () {
    ...
    var inputMap = Object.keys(this.state.settings).map(function (settingName) {
      ...
      return (
        <div key={settingName} style={{display: "block", padding: 5, whiteSpace: "nowrap"}}>
          <button style={this.getButtonStyles(resetButtonActive)} disabled={!resetButtonActive} onClick={this.resetValue.bind(this, settingName)}>Reset</button>
          <label style={this.getLabelStyles()}>
            ...
            />
            <span style={{margin: "0 5px 0 0"}}>{settingName}</span>
          </label>
        </div>
      );
    }.bind(this));
    ...
    return (
      <div {...this.getBrowserStateEvents()} style={this.buildStyles(this.getPaneStyles())}>
        <button onClick={this.togglePane} style={{position: "absolute", top: 8, left: "100%", padding: "10px 5px", fontSize: 16, backgroundColor: "#fff", backgroundImage: "none", border: "1px solid #999", borderRadius: "0 4px 4px 0", cursor: "pointer", outline: "none"}}>{this.state.paneExpanded ? "<<" : ">>"}</button>
        <h3 style={{textAlign: "center", margin: 0, padding: 8, borderBottom: "1px solid #999"}}>Settings</h3>

        <div style={{height: 300, overflowY: "scroll", overflowX: "scroll"}}>
          {inputMap}
        </div>
        <div style={{position: "absolute", display: "block", height: 60, width: 400, bottom: 0, borderTop: "1px solid #999"}}>
          <button style={assign(this.getButtonStyles(settingsHaveChanged), {padding: "8px 20px", margin: "11px 40px", float: "left"})} disabled={!settingsHaveChanged} onClick={this.saveSettings}>Save</button>
          <button style={assign(this.getButtonStyles(settingsHaveChanged), {padding: "8px 20px", margin: "11px 40px", float: "right"})} disabled={!settingsHaveChanged} onClick={this.resetAll}>Reset all</button>
        </div>
      </div>
    );
  }
